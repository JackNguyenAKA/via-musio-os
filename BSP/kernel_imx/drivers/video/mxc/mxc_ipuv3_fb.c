...
extern phys_addr_t vab820_fbaddr; 
			  ...
			  struct fb_info *fbi_to, bool both_on)
{
	...
	fbi = mxc_fbi_from->fbi;
	mxc_fbi_from->fbi = mxc_fbi_to->fbi;
	mxc_fbi_to->fbi = fbi;
...
static int mxcfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	if(mxc_fbi->ipu_ch == MEM_BG_SYNC && vab820_fbaddr != NULL) {
		var->xres_virtual = 4096;
		var->yres_virtual = 4096;
	} else {
...
extern void via_hdmi_hotplug(void);

static void mxcfb_work(struct work_struct *work)
{
	struct mxcfb_info *mxc_fbi = container_of(work, struct mxcfb_info, work);
	printk(KERN_WARNING "mxcfb_work start\n");

	via_hdmi_hotplug();

	msleep(3000);
	printk(KERN_WARNING "mxcfb_work end\n");
...
static irqreturn_t mxcfb_irq_handler(int irq, void *dev_id)
{
	...
	if(irq == IPU_IRQ_IDMAC_NFB4EOF_ERR23) {
		if(running == 0) {
			running = 1;
			INIT_WORK(&mxc_fbi->work, mxcfb_work);
			schedule_work(&mxc_fbi->work);
		}
		return IRQ_HANDLED;
	}
...
static int mxcfb_probe(struct platform_device *pdev)
{
	...
	} else if (vab820_fbaddr != NULL) {
		if (num_registered_fb == 0){
			fbi->fix.smem_len = SZ_128M;
			fbi->fix.smem_start = vab820_fbaddr;
			fbi->screen_base = ioremap(fbi->fix.smem_start, fbi->fix.smem_len);
			fbi->fix.reserved[0] = 0x17;
			fbi->fix.reserved[1] = 0x334;
