...
//#define ADAU1761_DEBUG_DETAIL 1
#ifdef ADAU1761_DEBUG_DETAIL
...
static int pwn_micbias;

#if 0
static void adau1761_default_setting(struct snd_soc_codec *codec)
{
	struct adau *adau = snd_soc_codec_get_drvdata(codec);

	/* enable clock */
	regmap_write(adau->regmap, 0x40F9, 0x7F);
	/* enable CLK0 */
	regmap_write(adau->regmap, 0x40FA, 0x1);
	
	/* enable power for L&R channel */
	regmap_write(adau->regmap, ADAU17X1_PLAY_POWER_MGMT, 0x03);
	/* disable HP */
	regmap_write(adau->regmap, 0x4024, 0x0);

	/* LINNG */
	regmap_write(adau->regmap, 0x400A, 0x1);
	/* RINNG */
	regmap_write(adau->regmap, 0x400C, 0x1);
	/* LDBOOST */
	regmap_write(adau->regmap, 0x400B, 0x10);
	/* RDBOOST */
	regmap_write(adau->regmap, 0x400D, 0x10);
	/* MBIEN */
	regmap_write(adau->regmap, 0x4010, 0x01);
	/* enable L&R ADC */
	regmap_write(adau->regmap, 0x4019, 0x03);

	/* MX3LM & disable MX3 */
	regmap_write(adau->regmap, 0x401C, 0x20);
	/* MX4RM & disable MX4 */
	regmap_write(adau->regmap, 0x401E, 0x40);
	/* MX5G4 & MX5G3 */
	regmap_write(adau->regmap, 0x4020, 0x0B);
	/* disable MX6G4 & MX6G3 */
	regmap_write(adau->regmap, 0x4021, 0x0);
	/* LOUTVOL */
	regmap_write(adau->regmap, 0x4025, 0xE6);
	/* ROUTVOL */
	regmap_write(adau->regmap, 0x4026, 0xE6);
	/* enable L&R DAC */
	regmap_write(adau->regmap, 0x402A, 0x03);

	/*regmap_write(adau->regmap, ADAU17X1_DSP_RUN, 0x01);*/
}
#endif
...
static int adau1761_setup_digmic_jackdetect(struct snd_soc_codec *codec)
{
	...
	DBG_DETAIL("val=0x%x", val);
	ret = regmap_write(adau->regmap, ADAU1761_DIGMIC_JACKDETECT, val);
	if (ret < 0) {
		printk("*** %s, regmap_write fail ***\n", __func__);
		return ret;
	}
...
static int adau1761_codec_probe(struct snd_soc_codec *codec)
{
	...
	if (pdata && pdata->input_differential) {
		...
		if (ret) {
			DBG_DETAIL("add codec controls(differential) fail");
			return ret;
		}	
	} else {
#if 0	
		...
		if (ret) {
			DBG_DETAIL("add codec controls(single) fail");
			return ret;
		}
#endif		
	}
	...
	if (ret) {
		DBG_DETAIL("setup headphone mode fail");
		return ret;
	}	
#if 0
	ret = adau1761_setup_digmic_jackdetect(codec);
	if (ret) {
		DBG_DETAIL("setup digmic jackdetect fail");
		return ret;
	}	
#endif
	DBG_DETAIL();
	if (adau->type == ADAU1761) {
		...
		if (ret) {
			DBG_DETAIL("dapm new controls fail");
			return ret;
		}	
		...
		if (ret) {
			DBG_DETAIL("dapm add routes fail");
			return ret;
		}	
#endif	
	...
	if (ret < 0) {
		printk("*** %s, add_routes fail ***\n", __func__);
		return ret;
	}	
#endif
	/*adau1761_default_setting(codec);*/
	return 0;
}
	...
	enum adau17x1_type type, void (*switch_mode)(struct device *dev))
{
	...
	if (ret) {
		DBG_DETAIL("adau17x1_probe fail");
		return ret;
	}	

	/* request power enable pin for mic-bias */
	pwn_micbias = of_get_named_gpio(dev->of_node, "pwn-micbias", 0);
	if (!gpio_is_valid(pwn_micbias)) {
		printk("*** %s, no mic-bias pwn pin available ***\n", __func__);
		return -EINVAL;
	}
	ret = devm_gpio_request_one(dev, pwn_micbias, GPIOF_OUT_INIT_LOW,
					"adau1761_pwn");
	if (ret < 0) {
		printk("*** %s, gpio_request_one fail ***\n", __func__);
		return ret;
	}
	gpio_set_value(pwn_micbias, 1);
