...
//#define ADAU17x1_DEBUG_DETAIL 1
#ifdef ADAU17x1_DEBUG_DETAIL
...
static void adau17x1_regs_dump(struct snd_soc_codec *codec)
{
	...
	printk(KERN_INFO ">>>>>>>>>>>>>>>>>>>>>>");
	for( ; i < 0x40FB; i++) {
		if ((i == 0x4001) || (i == 0x402E) || (i == 0x40C5))
			continue;
		/*printk(KERN_INFO "reg[%02x] = %02x\n", i, snd_soc_read(codec, i));*/
		ret = regmap_read(adau->regmap, i, &val);
		if (ret)
			printk("*** %s, regmap_read fail 0x%x ***\n", __func__, i);
		else
			printk(KERN_INFO "reg[%02x] = %02x\n", i, val);
		if (i == 0x4031)
			i = 0x4035;
...
static void adau17x1_default_setting(struct snd_soc_codec *codec, int stream)
{
	struct adau *adau = snd_soc_codec_get_drvdata(codec);
	...
	if (stream == SNDRV_PCM_STREAM_CAPTURE) {
		...
		/* Record Volume Ctrl Left */
		regmap_write(adau->regmap, 0x400E, 0xAF);
		/* Record Volume Ctrl Right */
		regmap_write(adau->regmap, 0x400F, 0xAF);
		/* enable L&R ADC */
		...
		/* disable MX5G4 & MX5G3 */
		regmap_write(adau->regmap, 0x4020, 0x0);
		/* MX6G4 & MX6G3 */
		regmap_write(adau->regmap, 0x4021, 0x0B);
		/* LOUTVOL */
		regmap_write(adau->regmap, 0x4025, 0xE4);
		/* ROUTVOL */
	...
	struct snd_pcm_hw_params *params, struct snd_soc_dai *dai)
{
	...
	if (adau->clk_src == ADAU17X1_CLK_SRC_PLL) {
		printk("*** %s, ADAU17X1_CLK_SRC_PLL ***\n", __func__);
		freq = adau->pll_freq;
	} else
		freq = adau->sysclk;

	DBG_DETAIL("freq=%d, rate=%d", freq, params_rate(params));
	...
	if (adau17x1_has_dsp(adau)) {
		...
		if (ret < 0) {
			printk("*** %s, setup_firmware fail ***\n", __func__);
			return ret;
		} else
			DBG_DETAIL("setup_firmware ok");
	}

	if (adau->dai_fmt != SND_SOC_DAIFMT_RIGHT_J) {
		adau17x1_default_setting(codec, substream->stream);
#if 0		
		regmap_write(adau->regmap, 0x40F9, 0x7F);
		/* MX3LM & disable MX3 */
		regmap_write(adau->regmap, 0x401C, 0x20);
		...
		regmap_write(adau->regmap, 0x4022, 0x0);
		regmap_write(adau->regmap, 0x4023, 0x0);
		
		regmap_write(adau->regmap, ADAU17X1_DSP_RUN, 0x01);
#endif		
		/*adau17x1_regs_dump(codec);*/
		return 0;
	}	
	...
	ret = regmap_update_bits(adau->regmap, ADAU17X1_SERIAL_PORT1,
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	if (adau->clk_src != clk_id) {
		if (clk_id == ADAU17X1_CLK_SRC_PLL) {
			DBG_DETAIL("ADAU17X1_CLK_SRC_PLL");
			snd_soc_dapm_add_routes(dapm,
				&adau17x1_dapm_pll_route, 1);
		} else {
			DBG_DETAIL("ADAU17X1_CLK_SRC_MCLK");
