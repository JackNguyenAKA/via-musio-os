...
static int soc_bind_dai_link(struct snd_soc_card *card, int num)
{
	...
	list_for_each_entry(codec, &codec_list, list) {
		...
		/*if((dai_link->codec_of_node && g_audio_device == 0) 
		    || (dai_link->codec_name && g_audio_device == 1)) {
		    if (g_audio_device)
		    	printk("*** Got Codec, %s ***\n", dai_link->codec_name);
			else
...
static int __init set_audio_output(char *options)
{
...
__setup("audio=", set_audio_output);
