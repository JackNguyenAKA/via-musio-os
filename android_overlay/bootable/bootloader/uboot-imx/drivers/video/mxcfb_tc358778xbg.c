...
static void tc358778xbg_read_word(int index, u16 *val)
{
	u8 *pu8Rd;
#if 1
	if (i2c_adpt->read(i2c_adpt, 0xe, index, 2, (char *) val, 2)) {
		printf("*E* %s(0x%x,0x%x)\n", __func__, index, *val);
		return;
	}

	pu8Rd = (u8 *) val;
//	printf("[sam] read word(0x%x,0x%x)-->%x,%x %x\n", index, *val,
//						index, pu8Rd[0], pu8Rd[1]);
#else
	u8 au8RegBuf[2] = {0};
	...
	pu8Rd = (u8 *) val;
#ifdef DEBUG_DETAIL
	printf("[sam] read word(0x%x,0x%x),%x %x\n", index, *val,
						pu8Rd[0], pu8Rd[1]);
#endif
...
static void tc358778xbg_write_word(int index, u16 val)
{
	...
	au8Buf[1] = val & 0xff;
#if 0	
	printf("[sam] write word(0x%x,0x%x)->(0x%x,0x%x,0x%x)\n",
		index, val, index, au8Buf[0], au8Buf[1]);
#endif
	if (i2c_adpt->write(i2c_adpt, 0xe, index, 2, (char *) au8Buf, 2)) {
		printf("*E* %s(0x%x,0x%x)\n", __func__, index, val);
	}
#if 0
	{
		u16 temp = 0;
		tc358778xbg_read_word(index,&temp);
		printf("[sam] read(0x%x,0x%x)\n", index, temp);
	}
#endif
...
#ifdef DEBUG_DETAIL
	printf("i2cset -f -y 2 0x0E 0x%02X 0x%02X 0x%02X 0x%02X i\n",
