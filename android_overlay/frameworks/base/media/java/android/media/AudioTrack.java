...
public class AudioTrack
{
    ...
    */
	...
	if(packageName!=null && packageName.contains("quicksearch")){
            setVolume(0);
	    Log.d(TAG, "setVolume(0) @ play()");
	} 
        ...
        synchronized(mPlayStateLock) {
		if(packageName!=null && packageName.contains("quicksearch")){
			Log.d(TAG, "stop() called right after play() called");
			if(getPlayState() == PLAYSTATE_PLAYING) native_pause();
			native_flush();
			native_release();
		} else {
	    		native_start();
