...
static void select_output_device(struct imx_audio_device *adev)
{
    ...
    ALOGV("headphone %d ,headset %d ,speaker %d, earpiece %d, bt %d \n", headphone_on, headset_on, speaker_on, earpiece_on, bt_on);
    /* select output stage */
    ...
    if (pcm_device && (adev->active_output[OUTPUT_ESAI] == NULL || adev->active_output[OUTPUT_ESAI]->standby)) {
       ...
       if (out->device & AUDIO_DEVICE_OUT_DGTL_DOCK_HEADSET) {
        ...
        if (out->device & AUDIO_DEVICE_OUT_ALL_SCO) {
            ALOGV("*** SCO Out, %d %d ***", adev_get_rate_for_device(adev, out->device, PCM_OUT), adev->default_rate);
            int ret_sco;
            release_resampler(out->resampler[PCM_NORMAL]);
            out->config[PCM_NORMAL].rate = 8000;
            out->config[PCM_NORMAL].channels = 1;

            ret_sco = create_resampler(adev->default_rate,
                        ...
                        &out->resampler[PCM_NORMAL]);
            if (ret_sco != 0) {
                ALOGE("can not create_resampler for SCO, inRate %d, outRate 8000, channels 2, device 0x%x",
                    adev->default_rate, out->device);
                return ret_sco;
            }
            port = 1;
        }
    ...
    for (i = 0; i < PCM_TOTAL; i++) {
        if (out->pcm[i]) {
            if (out->config[i].rate == adev->default_rate) {
                ...
                if (out->device & AUDIO_DEVICE_OUT_ALL_SCO) {
                    /* Mono output */
                    channel_buf = (int16_t *)out->buffer;
                    tmp_buf = (int16_t *)out->buffer;
                    frame_size = frame_size / 2;
                    sample_cnt = out_frames * frame_size;
                    for (j = 0; j < sample_cnt; j++)
                        channel_buf[j] = tmp_buf[2 * j];
                }
...
static int start_input_stream(struct imx_stream_in *in)
{
    ...
    } else if (in->device & AUDIO_DEVICE_IN_ALL_SCO) {
        ALOGV("*** SCO In, %d %d ***", adev_get_rate_for_device(adev, in->device, PCM_IN), adev->default_rate);
        ALOGV("*** requested_rate %d, requested_channel %d ***", in->requested_rate, in->requested_channel);
        port = 1;
        in->config.rate = 8000;
        in->config.channels = 1;
    }
    ...
    ALOGW("rate %d, channel %d format %d, period_size 0x%x, period_count 0x%x", in->config.rate, in->config.channels, 
                                 in->config.format, in->config.period_size, in->config.period_count);
