...
int via_lcd_cmd_disp = 1;
int via_lcd_cmd_sleep = 1;
int via_early_blank = 1;
...
void tc358778xbg_poweron_sequence(void)
{
#if 0
...
void tc358778xbg_poweroff_sequence(void)
{
	gpio_request(MX6Q_DISPRESET_GPIO6_11, "gpio6_11");
	gpio_direction_output(MX6Q_DISPRESET_GPIO6_11, 0);
	...
	/* display power enable */
	gpio_request(MX6Q_DISPPWREN_GPIO6_14, "gpio6_14");
	gpio_direction_output(MX6Q_DISPPWREN_GPIO6_14, 0);
	mdelay(1);

	/* display VCC power enable */
	gpio_request(MX6Q_DISPPWREN_GPIO6_16, "gpio6_16");
	gpio_direction_output(MX6Q_DISPPWREN_GPIO6_16, 0);
	mdelay(1);
				...
				void *buffer, size_t *len, loff_t *ppos)
{
	...
	if (!write) {
		...
		case 6:
			...
			break;
		case 8:
			via_proc_value = via_lcd_cmd_sleep;
			break;
		case 9:
			via_proc_value = via_lcd_cmd_disp;
			break;
		default:
			break;
		}
	...
	if (write) {
		...
		case 8:
			...
			break;
		case 9:
			via_lcd_cmd_disp = via_proc_value;
			break;
		case 10:
			if (via_proc_value) {
				tc358778xbg_poweron_sequence();
			} else {
				tc358778xbg_poweroff_sequence();
			}
			break;
		case 11:
			via_early_blank = via_proc_value;
			break;
		default:
			break;
		}
